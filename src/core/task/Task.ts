...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
    ...
    /**
     * Disposes of resources used by the Task
     * kilocode_change - Added dispose method for proper cleanup of timers and resources
     */
    public dispose(): void {
        // Clear any pending pause interval
        if (this.pauseInterval) {
            clearInterval(this.pauseInterval)
            this.pauseInterval = undefined
        }

        // Clear any pending timeouts from processQueuedMessages
        // Note: Since setTimeout is used with 0 delay in processQueuedMessages,
        // we can't easily track all timeout IDs, but the abort flag
        // should prevent them from causing issues after disposal
        this.abort = true
        ...
        // Clear references
        this.providerRef = undefined as any
    }
}
