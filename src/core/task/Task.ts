...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
    ...
    private isDisposed: boolean = false // kilocode_change
    private memoryReleased: boolean = false // kilocode_change
    userMessageContent: (
    ...
    public async abortTask(isAbandoned = false) {
        ...
        try {
            ...
            this.releaseTaskMemory() // kilocode_change
        }
    }
    ...
    public dispose(): void {
        ...
        this.isDisposed = true // kilocode_change
        ...
        if (this.rooProtectedController) {
            this.rooProtectedController = undefined // kilocode_change
        }
        ...
        if (this.checkpointService) {
            try {
                this.checkpointService.removeAllListeners()
            } catch (error) {
                console.error("Error disposing checkpoint service:", error) // kilocode_change
            }

            this.checkpointService = undefined // kilocode_change
            this.checkpointServiceInitializing = false // kilocode_change
        }
        ...
        try {
            ...
            void this.diffViewProvider.reset().catch((error) =>
                console.error("Error resetting diff view provider:", error),
            ) // kilocode_change
        } catch (error) {
            console.error("Error resetting diff view provider:", error)
        }
    ...
    private releaseTaskMemory(): void {
        if (this.memoryReleased) {
            return
        }

        this.memoryReleased = true // kilocode_change
        this.apiConversationHistory = [] // kilocode_change
        this.clineMessages = [] // kilocode_change
        this.assistantMessageContent = [] // kilocode_change
        this.userMessageContent = [] // kilocode_change
        this.toolUsage = {} // kilocode_change
        this.tokenUsageSnapshot = undefined // kilocode_change
        this.tokenUsageSnapshotAt = undefined // kilocode_change

        try {
            this.assistantMessageParser.reset()
        } catch (error) {
            console.error("Error resetting assistant message parser during cleanup:", error) // kilocode_change
        }
    }
