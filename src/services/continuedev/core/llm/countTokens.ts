...
interface Encoding {
    ...
    // kilocode_change: Add free method to prevent memory leaks
    free?: () => void
}
...
// kilocode_change: Cache encoding to prevent memory leaks from repeated WASM instance creation
let gptEncoding: Tiktoken | null = null
const llamaEncoding = new LlamaEncoding()

// kilocode_change: Cleanup function to free encoding resources and prevent memory leaks
...
// kilocode_change: Return Tiktoken directly to maintain free() method access
export function encodingForModel(modelName: string): Tiktoken | LlamaEncoding {
    if (!modelUsesGptTokenizer(modelName)) return llamaEncoding
    // kilocode_change: Cache encoding instance to prevent memory leaks from repeated WASM initialization
