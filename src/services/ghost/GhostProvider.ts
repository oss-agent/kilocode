...
export class GhostProvider {
    ...
    // kilocode_change start: Fix memory leak by storing disposables for event listeners
    private eventListenerDisposables: vscode.Disposable[] = []
    // kilocode_change end
    ...
    private constructor(context: vscode.ExtensionContext, cline: ClineProvider) {
        ...
        // kilocode_change start: Store event listener disposables to prevent memory leaks
        // Register document event handlers
        this.eventListenerDisposables.push(
        ...
        // Also push to context.subscriptions for automatic cleanup on extension deactivation
        context.subscriptions.push(...this.eventListenerDisposables)
        // kilocode_change end
    ...
    public dispose(): void {
        ...
        // kilocode_change start: Dispose registered event listeners to prevent leaks
        for (const disposable of this.eventListenerDisposables.splice(0)) {
            try {
                disposable.dispose()
            } catch (error) {
                console.error("Failed to dispose ghost provider event listener:", error)
            }
        }
        // kilocode_change end
