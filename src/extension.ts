...
export async function deactivate() {
    ...
    // kilocode_change: Clean up cloud service event handlers to prevent memory leaks
    ...
    // kilocode_change: Free tokenizer encoding resources to prevent WASM memory leaks
    try {
        const { freeEncodingResources } = await import("./services/continuedev/core/llm/countTokens")
        freeEncodingResources()
        outputChannel.appendLine("Tokenizer encoding resources freed")
    } catch (error) {
        outputChannel.appendLine(
            `Failed to free tokenizer resources: ${error instanceof Error ? error.message : String(error)}`,
        )
    }
}
