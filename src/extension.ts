...
export async function deactivate() {
    ...
    try {
        for (const instance of (ClineProvider as any).activeInstances) {
            await instance.dispose()
        }
        outputChannel.appendLine("ClineProvider instances disposed")
    } catch (error) {
        outputChannel.appendLine(
            `Failed to dispose ClineProvider instances: ${error instanceof Error ? error.message : String(error)}`,
        )
    }
    ...
    try {
        ...
        outputChannel.appendLine("CodeIndexManager instances disposed")
    } catch (error) {
        outputChannel.appendLine(
