...
const App = () => {
	...
	// Create MemoryService instance once using useRef to prevent recreation on re-renders
	...
	// Separate effect for memory service lifecycle
	useEffect(() => {
		if (didHydrateState) {
			if (!memoryServiceRef.current) {
				memoryServiceRef.current = new MemoryService()
				memoryServiceRef.current.start()
			}

			return () => {
				if (memoryServiceRef.current) {
					memoryServiceRef.current.stop()
	...
	useEffect(() => {
		// Store handlers so they can be cleaned up
		let errorHandler: ((event: ErrorEvent) => void) | null = null
		let rejectionHandler: ((event: PromiseRejectionEvent) => void) | null = null

		// Initialize source maps for better error reporting in production
		if (process.env.NODE_ENV === "production") {
			const { errorHandler: eh, rejectionHandler: rh } = initializeSourceMaps()
			errorHandler = eh
			rejectionHandler = rh
		...
		return () => {
			...
			if (rejectionHandler) {
				window.removeEventListener("unhandledrejection", rejectionHandler as EventListener)
			}
		}
	}, [])
	}, [])
